report 50316 "Aged Accounts Receivable No."
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src\Report Layout\Aged Accounts Receivable New.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Receivable No.of Days';
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(TempCustLedgEntryLoop; "Integer")
                    {
                        ...
                        begin
                            ...
                            NumberOfLines += 1;
                            // Message(CustLedgEntryEndingDate."Global Dimension 2 Code");
                            //<<PCPL-064 4aug2023
                            Clear(StoreNo);
                            if CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::Invoice then begin
                                SIH.Reset();
                                SIH.SetRange("No.", CustLedgEntryEndingDate."Document No.");
                                if SIH.FindFirst() then
                                    if SIH."Store No." <> '' then
                                        StoreNo := SIH."Store No."
                                    else
                                        StoreNo := CustLedgEntryEndingDate."Global Dimension 2 Code"
                                else begin
                                    StoreNo := CustLedgEntryEndingDate."Global Dimension 2 Code";
                                end;
                                ...
                                if CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::"Credit Memo" then begin
                                    SCMH.Reset();
                                    SCMH.SetRange("No.", CustLedgEntryEndingDate."Document No.");
                                    if SCMH.FindFirst() then
                                        if SCMH."Store No." <> '' then
                                    ...
                                    if CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::Payment then begin
                                        StoreNo := CustLedgEntryEndingDate."Global Dimension 2 Code";
                            ...
                            if CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::Invoice then begin
                                SIL.Reset();
                                SIL.SetRange("Document No.", CustLedgEntryEndingDate."Document No.");
                                ...
                                if CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::"Credit Memo" then begin
                                    SCML.Reset();
                                    SCML.SetRange("Document No.", CustLedgEntryEndingDate."Document No.");
                                    if SCML.FindFirst() then Begin
                                        SalesPersonCode := SCML."Salesperson Name";
                                    End;
                                end;  //8aug2023 
                            // end;
                            NoDays := EndingDate - CustLedgEntryEndingDate."Posting Date";  //No.OfDays 4Aug2023
                                                                                            //>>PCPL-064 4aug2023
                        end;
